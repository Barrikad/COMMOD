...
public class Search {
    ...
    public static void main(String[] argv) {
        try {
            ...
            int x = 0;
            int y = len / ntasks;
            taskList.add(new SearchTask(text,pattern,x,y));
            for(int i = 1; i < ntasks - len % ntasks; i++) {
            	x = y - pattern.length + 1;
            	y = x + (len / ntasks) + pattern.length - 1;
            	taskList.add(new SearchTask(text,pattern,x,y));
            }
            for(int i = ntasks - len % ntasks; i < ntasks; i++) {
            	x = y - pattern.length + 1;
            	y = x + (len / ntasks) + pattern.length;
            	taskList.add(new SearchTask(text,pattern,x,y));
            }
            ...
            writeData(ntasks + ";" + nthreads + ";" + (singleTime / multiTime));
