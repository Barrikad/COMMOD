...
let AST2PG ast deterministic =
    ...
    let nextFreeNode = highestNode <- highestNode + 1
                       highestNode
    ...
    let rec expandCommand qs com qe =
        match com with
              ...
              | Coms(com1,com2) -> let qm = MidNode(nextFreeNode)
    ...
    and expandGCommand qs gcom qe = 
        match gcom with
              | Gives(bol,com) -> let qm  = MidNode(nextFreeNode)
                                  Set.union (Set.add (qs,BoolAct(bol),qm) Set.empty) (expandCommand qm com qe)
              | Else(gcom1,gcom2) -> if deterministic then
                                        let qm = MidNode(nextFreeNode)
